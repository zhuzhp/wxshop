<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:63:"D:\www\wxshop\public/../application/admin\view\order\index.html";i:1491976960;s:63:"D:\www\wxshop\public/../application/admin\view\public\base.html";i:1491877418;s:62:"D:\www\wxshop\public/../application/admin\view\public\nav.html";i:1491788115;}*/ ?>
<!DOCTYPE html>
...
<title>
<?php echo $title; ?>列表
<?php $groupnav = 'Orderinfo'; $activenav = 'Order/index'; ?>
        ...
        <div class="content-grid">
            <form id="orderForm" action="<?php echo url($tablename.'/index'); ?>" method="get">
                <div class="form-inline">
                    ...
                    <div class="input-group push-5">
                        <input type="text" class="form-control input-sm" placeholder="搜索订单编号|联系人|电话" name="key" value="<?php echo isset($params['key'])?$params['key']:''; ?>">
                    ...
                    <select class="form-control input-sm" name="pay_status" onchange="$('#orderForm').submit();">
                        <?php if(empty($params['pay_status']) || (($params['pay_status'] instanceof \think\Collection || $params['pay_status'] instanceof \think\Paginator ) && $params['pay_status']->isEmpty())): ?><option value="">--请选择支付状态--</option><?php else: ?><option value="<?php echo $params['pay_status']; ?>"><?php echo $pay_status[$params['pay_status']]; ?></option><?php endif; if(is_array($pay_status_list) || $pay_status_list instanceof \think\Collection || $pay_status_list instanceof \think\Paginator): $i = 0; $__LIST__ = $pay_status_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pay): $mod = ($i % 2 );++$i;if($pay['id'] != $params['pay_status']): ?>
                        <option value="<?php echo $pay['id']; ?>"><?php echo $pay['status']; ?></option>
                        <?php endif; endforeach; endif; else: echo "" ;endif; if(!(empty($params['pay_status']) || (($params['pay_status'] instanceof \think\Collection || $params['pay_status'] instanceof \think\Paginator ) && $params['pay_status']->isEmpty()))): ?><option value="">--请选择支付状态--</option><?php endif; ?>
                    </select>
                    <select class="form-control input-sm" name="ship_status" onchange="$('#orderForm').submit();">
                        <?php if(empty($params['ship_status']) || (($params['ship_status'] instanceof \think\Collection || $params['ship_status'] instanceof \think\Paginator ) && $params['ship_status']->isEmpty())): ?><option value="">--请选择发货状态--</option><?php else: ?><option value="<?php echo $params['ship_status']; ?>"><?php echo $ship_status[$params['ship_status']]; ?></option><?php endif; if(is_array($ship_status_list) || $ship_status_list instanceof \think\Collection || $ship_status_list instanceof \think\Paginator): $i = 0; $__LIST__ = $ship_status_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ship): $mod = ($i % 2 );++$i;if($ship['id'] != $params['ship_status']): ?>
                        <option value="<?php echo $ship['id']; ?>"><?php echo $ship['status']; ?></option>
                        <?php endif; endforeach; endif; else: echo "" ;endif; if(!(empty($params['ship_status']) || (($params['ship_status'] instanceof \think\Collection || $params['ship_status'] instanceof \think\Paginator ) && $params['ship_status']->isEmpty()))): ?><option value="">--请选择发货状态--</option><?php endif; ?>
                    </select>
                ...
                <div class="row">
                    <table class="js-table-checkable table table-hover">
                        ...
                        <tr class="active">
                            <td><p class="font-w600 push-5"><?php echo $vo['order_number']; ?></p></td>
                            <td><p class="text-muted remove-margin-b"><?php echo $vo['total_amount']; ?></p></td>
                            <td class="hidden-xs"><em class="text-muted"><?php echo $vo['discounted_price']; ?></em></td>
                            <td class="hidden-xs"><em class="text-muted"><?php echo $vo['amounts_payable']; ?></em></td>
                            <td class="hidden-xs"><em class="text-muted"><?php echo $vo['pay_status']; ?></em></td>
                            <td class="hidden-xs"><em class="text-muted"><?php echo $vo['ship_status']; ?></em></td>
                            <td class="hidden-xs"><em class="text-muted"><?php echo $vo['contant_person']; ?></em></td>
                            ...
                            <td>
                                <a href="<?php echo url($tablename.'/goodslist',['id'=>$vo['id']]); ?>" class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="" data-original-title="查看订单商品"><i class="glyphicon glyphicon-zoom-in fa-1x"></i></a>

<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:62:"D:\www\wxshop\public/../application/admin\view\area\index.html";i:1491978649;s:63:"D:\www\wxshop\public/../application/admin\view\public\base.html";i:1491877418;s:62:"D:\www\wxshop\public/../application/admin\view\public\nav.html";i:1491788115;}*/ ?>
<!DOCTYPE html>
...
<title>
<?php echo $title; ?>列表
<?php $groupnav = 'System'; $activenav = 'Area/index'; ?>
...
<div class="bg-white">
    <section class="content content-boxed overflow-hidden">
        <div class="content-grid">
            <form id="sortForm" action="<?php echo url('area/modifylistupdatesort'); ?>" method="post">
                ...
                <div class="form-inline">
                    ...
                    <div class="input-group push-5">
                        <input type="text" class="form-control input-sm" placeholder="搜索地区名称" value="<?php echo isset($params['key'])?$params['key']:''; ?>">
          <span class="input-group-btn">
            <button class="btn btn-sm btn-default" type="button" onclick="var v=$(this).parent().prev().val(); location.href='<?php echo url('area/index', u_pam($params, null, array('page','key'))); ?>?key='+v;"> <i class="fa fa-search"></i> </button>
          </span>
                    </div>
                    <select class="form-control input-sm" onchange="location.href='<?php echo url('area/index', u_pam($params, null, array('page','pagesize'))); ?>?pagesize='+this.value">
                ...
                <div class="row">
                    <table class="js-table-checkable table table-hover">
                        ...
                        <tr class="list-box">
                            ...
                            <td>
                                <button onclick="addsoncate(<?php echo $vo['id']; ?>,'<?php echo $vo['catename']; ?>');" data-toggle="modal" data-target="#modal-son" class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="添加子地区" data-original-title="添加子地区"><i class="fa fa-plus fa-1x"></i></button>
                                ...
                                <a data-href="<?php echo url('area/delete',['id'=>$vo['id'],'controller'=>'area']); ?>" onclick="deleleCate(this)" class="btn btn-xs btn-default delete-one" type="button" data-toggle="tooltip" title="" data-original-title="删除"><i class="fa fa-times"></i></a>
                            </td>
                        </tr>
        ...
        <div class="modal-content">
            <div class="block block-themed block-transparent remove-margin-b">
                <div class="block-header bg-primary-dark">
                    ...
                    <h3 class="block-title">添加子地区</h3>
                </div>
                <div class="block-content">
                    <div class="block-content block-content-narrow">
                        <form class="js-validation form-horizontal push-10-t" action="<?php echo url('area/addsoncate'); ?>" method="post" novalidate="novalidate">
                            <div class="form-group">
                                <div class="col-sm-9">
                                    <div class="form-material">
                                        <input type="hidden" name="controller" value="area">
                                        <input type="hidden" name="catetype" value="area">
                                        <input type="hidden" id="par-id" name="id" val="">
                                        <input class="form-control" type="text" readonly  value="" id="parent_cate">
                                        <label for="val-name">父级地区</label>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-9">
                                    <div class="form-material">
                                        <input class="form-control" type="text" id="cat-name" name="catename" placeholder="输入地区名称.">
                                        <label for="val-name">地区名称</label>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
        ...
        <div class="modal-content">
            <div class="block block-themed block-transparent remove-margin-b">
                <div class="block-header bg-primary-dark">
                    ...
                    <h3 class="block-title">添加地区</h3>
                </div>
                <div class="block-content">
                    <div class="block-content block-content-narrow">
                        <form class="js-validation-role form-horizontal push-10-t" action="<?php echo url('area/modify'); ?>" method="post" novalidate="novalidate">
                            <div class="form-group">
                                <div class="col-sm-9">
                                    <div class="form-material">
                                        <input type="hidden" name="controller" value="area">
                                        ...
                                        <input class="form-control" type="text" id="val-name" name="catename" placeholder="输入地区名称.">
                                        <label for="val-name">地区名称</label>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    {
        $(".block-title").html('修改地区名称');
        $('#val-id').val(id);
        $('#val-name').val(catename);
    }
    function add()
    {
        $('.block-title').html('添加地区');
        $('#val-id').val('');
        $('#val-name').val('');
    }
    function Unfolded(obj,id,catename){
        ...
        if(index == 1){
            $.post('/admin/area/unfolded.html',{id,id},function(data){
                ...
                if(venjson){
                    ...
                    for(var i=0; i<venjson.length;i++){
                        ...
                        <td style="text-align:left;padding-left:'+padLeft+'px;"><i class="glyphicon glyphicon-menu-right fa-1x" data-index = "1" onclick="Unfolded(this,'+venjson[i].id+',\''+venjson[i].catename+'\')"></i></td>\
                         ...
                         <td>\
                            <button onclick="addsoncate('+venjson[i].id+',\''+venjson[i].catename+'\');" data-toggle="modal" data-target="#modal-son" class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="添加子地区" data-original-title="添加子地区"><i class="fa fa-plus fa-1x"></i></button>\
                            ...
                            <a data-href="/admin/area/delete/controller/area/id/'+venjson[i].id+'.html" onclick="deleleCate(this)" class="btn btn-xs btn-default delete-one" type="button" data-toggle="tooltip" title="删除" data-original-title="删除"><i class="fa fa-times"></i></a>\
                         </td>\
                       </tr>';
                    }
